%% Changes done by Hrishikesh:

%% replaced Bode plot script by:
%% Bode plot
...
%% replaced Pole zero plot with:
% fig1 = figure(1);
% 
% % Extract poles (p) and zeros (z) of the NTF (Noise Transfer Function)
% [z, p, k] = zpkdata(H, 'v');  % Extract zeros, poles, and gain as vectors
% 
% % Create a scatter plot for poles and zeros
% scatter(real(z), imag(z), 'o', 'DisplayName', 'Zeros', 'MarkerEdgeColor', 'b', 'LineWidth', 1.5);
% hold on;
% scatter(real(p), imag(p), 'x', 'DisplayName', 'Poles', 'MarkerEdgeColor', 'r', 'LineWidth', 1.5);
% 
% % Plot the unit circle
% theta = linspace(0, 2*pi, 500);  % Angle from 0 to 2π
...
% legend('Zeros', 'Poles', 'Unit Circle');  % Add legend for unit circle
% axis equal;  % Ensure equal scaling for x and y axes
% hold off;

% added axes labels, legends to figure 3
...
%% Pole-zero map (Manual Replacement for plotPZ)
fig1 = figure(1);
